...
import 'package:app/utils/environment.dart';
...
class PlaceApiProvider {
  ...
  Future<Suggestion> fetchMyLocation(lat, long) async {
    final lang = Platform.localeName;
    final request = 'https://maps.googleapis.com/maps/api/place/textsearch/json?query=a&location=$lat,$long&radius=10000&key=$apiKey';
    final response = await client.get(request);
  ...
  Future<dynamic> getPlaceDetailFromId(String placeId) async {
    ...
    final request = 'https://maps.googleapis.com/maps/api/place/details/json?fields=address_components&place_id=$placeId&key=$apiKey';
    final response = await client.get(request);
    String country;
    String countryCode;
    String province;
