...
class _YourAvailableState extends State<YourAvailable> {
  ...
  void initState() {
    ...
    if (widget.userDaysAvailable[0].dayOfTheWeek == 0 && widget.userDaysAvailable[0].available != null) {
      _sunday = true;
    }
    if (_sunday) {
      UserHoursAvailable available = UserHoursAvailable.fromJson(jsonDecode(widget.userDaysAvailable[0].available));
      this._userDaysAvailable.daysAvailable[0] = available;
    }
    if (widget.userDaysAvailable[1].dayOfTheWeek == 1 && widget.userDaysAvailable[1].available != null) {
      _monday = true;
    }
    if (_monday) {
      UserHoursAvailable available = UserHoursAvailable.fromJson(jsonDecode(widget.userDaysAvailable[1].available));
      this._userDaysAvailable.daysAvailable[1] = available;
    }
    if (widget.userDaysAvailable[2].dayOfTheWeek == 2 && widget.userDaysAvailable[2].available != null) {
      _tuesday = true;
    }
    if (_tuesday) {
      UserHoursAvailable available = UserHoursAvailable.fromJson(jsonDecode(widget.userDaysAvailable[2].available));
      this._userDaysAvailable.daysAvailable[2] = available;
    }
    if (widget.userDaysAvailable[3].dayOfTheWeek == 3 && widget.userDaysAvailable[3].available != null) {
      _wednesday = true;
    }
    if (_wednesday) {
      UserHoursAvailable available = UserHoursAvailable.fromJson(jsonDecode(widget.userDaysAvailable[3].available));
      this._userDaysAvailable.daysAvailable[3] = available;
    }
    if (widget.userDaysAvailable[4].dayOfTheWeek == 4 && widget.userDaysAvailable[4].available != null) {
      _thursday = true;
    }
    if (_thursday) {
      UserHoursAvailable available = UserHoursAvailable.fromJson(jsonDecode(widget.userDaysAvailable[4].available));
      this._userDaysAvailable.daysAvailable[4] = available;
    }
    if (widget.userDaysAvailable[5].dayOfTheWeek == 5 && widget.userDaysAvailable[5].available != null) {
      _friday = true;
    }
    if (_friday) {
      UserHoursAvailable available = UserHoursAvailable.fromJson(jsonDecode(widget.userDaysAvailable[5].available));
      this._userDaysAvailable.daysAvailable[5] = available;
    }
    if (widget.userDaysAvailable[6].dayOfTheWeek == 6 && widget.userDaysAvailable[6].available != null) {
      _saturday = true;
    }
    if (_saturday) {
      UserHoursAvailable available = UserHoursAvailable.fromJson(jsonDecode(widget.userDaysAvailable[6].available));
              ...
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () async {
                      if (_thursday) {
                        print(jsonDecode(widget.userDaysAvailable[4].available));
                        final UserHoursAvailable hoursAvailable = await showModalBottomSheet(
                          backgroundColor: Colors.transparent,
                          context: context,
                          enableDrag: false,
                  ...
                  child: FlatButton(
                    ...
                    onPressed: () async {
                      ...
                      if(noDaysSelected) {
                        ...
                        //     ProfileLoadingEvent()
                        // );
                        final Map<int, UserHoursAvailable> daysAvailable = this._userDaysAvailable.daysAvailable;

                        final editUserDaysAvailableResponse = await UserRepository().editUserDaysAvailable(
                          daysAvailable,
                        );

                        if (editUserDaysAvailableResponse['success'] == true) {
